...
> :memo: **Note**
>
> Up to Csound 6, this opcode was called *vdivv_i*.

## Syntax
=== "Modern"
    ``` csound-orc
    vdivvi(ifn1, ifn2, ielements [, idstoffset] [, isrcoffset])
    ```
...
_vdivvi_ divides two vectorial control signals, that is, each element of ifn1 is divided by the corresponding element of ifn2. Each vectorial signal is hosted by a table (ifn1 and ifn2). The number of elements contained in both vectors must be the same.
...
This opcode works at init time. There is an k-rate version of this opcode called [vdivv](../opcodes/vdivv.md).
