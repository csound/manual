...
-->
# unscheduleall
Removes all matching queued note events.
...
=== "Modern"
    ``` csound-orc
    unscheduleall(instrument, iStart, iDuration[, ip4, ip5, ...])
    ```
...
`unscheduleall` runs once at initialization and returns no value. Numeric values must match exactly. If nothing matches, nothing happens.

Every p-field must match. This opcode does not clear all events for an instrument.
...
[unschedule](unschedule.md), [schedule](schedule.md), [turnoff2](turnoff2.md), [Instrument Invocation](../control/invocat.md)
