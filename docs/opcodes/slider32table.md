...
_slider32table_ allows a bank of 32 different MIDI control message numbers.
...
_slider32table_ is very similar to  [slider32](../opcodes/slider32.md) and _sliderN_ family of opcodes (see their manual for more information). The actual difference is that the output is not stored to k-rate variables, but to a table, denoted by the _ioutTable_ argument. It is possible to define a starting index in order to use the same table for more than one slider bank (or other purposes).
