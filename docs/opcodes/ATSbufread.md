...
_ktimepnt_ – The time pointer in seconds used to index the ATS file. Used for _ATSbufread_ exactly the same as for [pvoc](../../opcodes/pvoc).
...
_ATSbufread_ is based on pvbufread by Richard Karpen. [ATScross](../../opcodes/ATScross), [ATSinterpread](../../opcodes/ATSinterpread) and [ATSpartialtap](../../opcodes/ATSpartialtap) are all dependent on _ATSbufread_ just as [pvcross](../../opcodes/pvcross) and [pvinterp](../../opcodes/pvinterp) are on [pvbufread](../../opcodes/pvbufread). _ATSbufread_ reads data from and ATS data file and stores it in an internal data table of frequency, amplitude pairs. The data stored by an _ATSbufread_ can only be accessed by other unit generators, and therefore, due to the architecture of Csound, an _ATSbufread_  must come before (but not necessarily directly) any dependent unit generator. Besides the fact that _ATSbufread_ does not output any data directly, it works almost exactly as [ATSadd](../../opcodes/ATSadd). The ugen uses a time pointer (_ktimepnt_) to index the data in time, _ipartials_, _ipartialoffset_ and _ipartialincr_ to select which partials to store in the table and _kfmod_ to scale partials in frequency.
...
See also the examples for [ATScross](../../opcodes/ATScross), [ATSinterpread](../../opcodes/ATSinterpread) and [ATSpartialtap](../../opcodes/ATSpartialtap)
