...
In one application, _tablera_ is intended to be used in pair with [tablewa](../opcodes/tablewa.md), or with several _tablera_ opcodes before a [tablewa](../opcodes/tablewa.md) -- all sharing the same _kstart_ variable.
...
_tablera_ starts reading from location _kstart_. _tablewa_ starts writing to location _kstart_, and then writes to _kstart_ with the number of the location one more than the one it last wrote. (Note that for [tablewa](../opcodes/tablewa.md), _kstart_ is both an input and output variable.) If the writing index reaches the end of the table, then no further writing occurs and zero is written to _kstart_.

For instance, if the table's length was 16 (locations 0 to 15), and _ksmps_ was 5. Then the following steps would occur with repetitive runs of the [tablewa](../opcodes/tablewa.md) opcode, assuming that _kstart_ started at 0.
...
*   The offset does not affect the number of read/write cycles performed, or the value written to _kstart_ by [tablewa](../opcodes/tablewa.md).
*   These opcodes cannot read or write the guardpoint. Use [tablegpw](../opcodes/tablegpw.md) to write the guardpoint after manipulations have been done with [tablewa](../opcodes/tablewa.md).
