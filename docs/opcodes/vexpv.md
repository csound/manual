...
_vexpv_ elevates each element of _ifn2_ to the corresponding element of _ifn1_. Each vectorial signal is hosted by a table (_ifn1_ and _ifn2_). The number of elements contained in both vectors must be the same.
...
vectors are processed). There is an i-rate version of this opcode called [vexpv_i](../opcodes/vexpv_i.md).
...
``` csound-csd title="Example of the vexpv opcode." linenums="1"
--8<-- "examples/vexpv.csd"
```
