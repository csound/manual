...
> :memo: **Note**
>
> Up to Csound 6, this opcode was called *vpowv_i*.

## Syntax
=== "Modern"
    ``` csound-orc
    vpowvi(ifn1, ifn2, ielements [, idstoffset] [, isrcoffset])
    ```
...
_vpowvi_ raises each element of ifn1 to the corresponding element of ifn2. Each vectorial signal is hosted by a table (ifn1 and ifn2). The number of elements contained in both vectors must be the same.
